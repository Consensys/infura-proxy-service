...
const fromBlockLimit = process.env.EVENT_FROM_BLOCK ? parseInt(process.env.EVENT_FROM_BLOCK) : 0
const getDecodedEvents = async (contract, eventBareName, inputs) => {
    let eventArray = await contract.queryFilter(eventBareName, fromBlockLimit)
    let decoded = []
    for (let i = 0; i < eventArray.length; i++) {
        const ev = eventArray[i];
        let decodedEvent = await contract.interface.decodeEventLog(eventBareName, ev.data, ev.topics)
        let o = {}
